...
// HostConsoleHandler handles console access requests for hosts
type HostConsoleHandler struct {
	logger      *slog.Logger
	hostsClient fulfillmentv1.HostsClient
}

// HostConsoleHandlerBuilder is the builder for HostConsoleHandler
type HostConsoleHandlerBuilder struct {
	logger     *slog.Logger
	grpcClient *grpc.ClientConn
}

// NewHostConsoleHandler creates a new HostConsoleHandlerBuilder
func NewHostConsoleHandler() *HostConsoleHandlerBuilder {
	return &HostConsoleHandlerBuilder{}
}

// SetLogger sets the logger for the host console handler
func (b *HostConsoleHandlerBuilder) SetLogger(logger *slog.Logger) *HostConsoleHandlerBuilder {
	b.logger = logger
	return b
}

// SetGrpcClient sets the gRPC client connection for the host console handler
func (b *HostConsoleHandlerBuilder) SetGrpcClient(grpcClient *grpc.ClientConn) *HostConsoleHandlerBuilder {
	b.grpcClient = grpcClient
	return b
}

// Build creates and returns the HostConsoleHandler instance
func (b *HostConsoleHandlerBuilder) Build() (*HostConsoleHandler, error) {
	...
	return &HostConsoleHandler{
		logger:      b.logger,
		hostsClient: fulfillmentv1.NewHostsClient(b.grpcClient),
	}, nil
}

// HandleConsole handles console access requests for hosts
func (h *HostConsoleHandler) HandleConsole(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()

	id := r.PathValue("id")
	if id == "" {
		h.logger.ErrorContext(ctx, "Missing Host id")
	...
	if err != nil {
		h.logger.ErrorContext(
			ctx,
			"Could not retrieve Host with id "+id,
			slog.Any("error", err),
		)
		http.Error(w, "Could not retrieve Host with id "+id, http.StatusInternalServerError)
		return
	}
	if response.Object == nil {
		...
		http.Error(w, "Host with id "+id+" does not exist", http.StatusNotFound)
		return
	}
	/*
	// TODO: verify the console is enabled for this host
	if response.Object.Status.ConsoleState != HostConsoleState_HOST_CONSOLE_STATE_ON {
		h.logger.ErrorContext(
			ctx,
			"The console of Host "+id+" is not enabled",
			slog.Any("error", err),
		)
		http.Error(w, "The console of Host "+id+" is not enabled", http.StatusForbidden)
		return
	}
	*/
	...
	/*
	// Start bidirectional copying between client and BMC
